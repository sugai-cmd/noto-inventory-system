...
| H | 売価 | 単価×本数×掛け率（自動計算） |
| I | 送料 | 送料計算機能で算出、またはマニュアル入力。**移植後は税込・50円繰り上げ後**の金額（DB_SCHEMA_DESIGN 20-3c） |
| J | 合計(税込) | 売価＋送料 …… **と書かれているが、実データはそうなっていない**（下記） |
...
**⚠️ J列（合計）は式が揃っていない。** 実データ117行を数えた結果：

| | 件数 | 実際の式 |
|---|---|---|
| 移行データ | **103件** | 売価 × 1.1（**送料を含んでいない**） |
| 移行データ | 14件 | (売価 ＋ 送料) × 1.1 |
| 移植後のコード | — | 売価 ＋ 送料（`orderService.submitOrder`。税を足さない） |

合計 3,609,521円は、売価＋送料（3,309,673円）ともその税込（3,640,640円）とも一致しない。
**そのため受注一覧の集計（PR F）はこの列を読まず、売価と送料から出し直している。**
消費税は売価にだけ掛ける（送料は既に税込のため）。
...
（移植後）資材の在庫は `v_material_stock` が資材在庫変動履歴から計算する。
K列の「段ボール外箱」に当たるぶんは、**発送済にしたときに減る**
（段ボール対応表の `material_id` で決まる。DB_SCHEMA_DESIGN 20-3b）。
D〜J列の個装（容器・キャップ・ラベル・紙垂・化粧箱など）は瓶詰め・箱詰めのレシピで減る。
この点は6章・7章で詳しく扱います。
...
| D | 商品名 | 送付した商品名。**販促資料だけを送った行は空**（実データ49件中2件） |
| E | 本数 | 送付本数。商品名が空の行では同じく空 |
...
**一覧を取るときの注意**：商品は **LEFT JOIN** で引くこと。内部結合にすると、
販促資料だけを送った行（商品が空）が黙って落ちる。実際に S2603-0001 と S2603-0002 の
2件が画面に出ていなかった（PR C-3 で修正）。
送付先も同様で、得意先マスタに無い送付先は `customer_id` が空のまま、名前は備考の
「送付先: …」に入っている（実データ49件中12件）。絞り込みは備考も見る必要がある。
...
|---|---|---|
| A | 日付 | 取引日 |
| B | 受払 | 「受入」「払出」、および棚卸で入る「棚卸調整」「欠減」（0019で追加） |
| C | 受入元 | 払出の場合：投入元タンクID |
| D | 受払量 | 常に正の数 |
| E | 払出先 | 受入の場合：受入先タンクID、払出の場合：蒸留ID |
| F | 原酒受払ID | `R{YYMM}-{帯}{連番3桁}`。**千の位はその月の何回目の移入か**（1回目 `1001〜`／2回目 `2001〜`）。同じ日付の受入は同じ回。回数は月ごとに1へ戻る。払出・棚卸調整・欠減は移入ではないので0帯（`0001〜`） |
| G | ID | 内部管理用ランダムID |
...
`submitRawSakeStocktaking()`（棚卸調整・欠減）、
`rawSakeLedgerService.updateRecord()` / `.cancelRecord()`（編集・取消。0020で追加）

**読み取り先**：`estimateRawSakeFifoLinks_()`（過去データ移行時のFIFO推定、通常運用では未使用）

**取消・編集について（0020）**：4つの台帳のうち**この台帳だけが取消の列を持っていません
でした**（商品・資材・浄酎は0006までに `is_cancelled` ほか3列が入っている）。そのため
原酒入荷や棚卸を打ち間違えても画面から直す手段がありませんでした。0020で同じ4列を足し、
蒸留タブに一覧・編集・取消を置いています。

- 台帳なので**物理削除はしない**。取消は `is_cancelled = 1` を立てるだけで、
  **数量は書き戻さない**（残量ビューが取消済みを0として数えるので、二重に戻す事故が起きない）
- 編集で直せるのは**日付・数量・備考**、受入ならさらに**銘柄・スペック・受入元**。
  **タンクと受払区分は直せません**（別の記録になるので取り消して入れ直す）
- **原酒受払IDは変わりません**。月をまたぐ日付に直しても `R2607-` のまま
- **`distillation_id` がある行はここから直せません**（422）。払出は全件が蒸留の記録で、
  蒸留明細を直すと払出行は取り消して入れ直されるため、ここで直しても食い違います
- 採番（`rawSakeCode.monthRows`）だけは**取消済みも読みます**。`lot_code` は UNIQUE なので、
  取り消しても番号は占有され続けます。外すと取り消した番号を採り直してINSERTが落ちます

**原酒ロットと蒸留の紐付け（0021）**：払出行は投入元タンクを持つだけで、
**そのタンクの中のどの受入ロットを使ったかは記録にありませんでした**
（ロット追跡は銘柄別に按分して推定していただけ）。瓶詰め→箱詰めの `wip_lot_allocations` と
同じ形の `raw_sake_lot_allocations` を足しています。

- 新しい蒸留は登録時に**古い順（FIFO）で自動に引き当て**ます。
  引き当てられなくても**蒸留は止めません**（移行データに在庫が足りない行が実在するため）
- **欠減も引き当てます**。減ったぶんをロットから引かないと、そのロットに残っていない
  液体を次の蒸留に引き当ててしまいます
- 過去分は画面のボタン（「古い順でまとめて引き当てる」）から埋めます。
  **2回押しても二重になりません**（足すのではなく置き換えるため）
- **引き当て済みの受入は取り消せず**（409）、**引当量より少ない数量には減らせません**（422）

**⚠️蒸留明細を差し替え・取消したときの見え方**：`updateDistillationDetail()` /
`cancelDistillationDetailItem()` は**元の払出行をそのまま残し**、打ち消しの
「戻しの受入」を別に足します（消すと残量が二重に戻るため）。
そのため台帳の行は `is_cancelled = 0` のままです。一覧では**蒸留明細の状態**
（`distillation_details.is_cancelled`）を見て、投入明細の画面と同じ言葉で出します。

| 行 | 一覧での状態 |
|---|---|
| 明細が取消済みの払出 | **取消済み**（明細の備考「修正のため差し替え → DTL-99」も出す） |
| 打ち消しの戻し受入 | **戻し**（受入に `distillation_id` が付くのはこれだけ） |
| 生きている払出 | 有効 |

**引き当ては外しません。** 差し替えられた払出は残量を引いたままで、戻しの受入が
足し返しているためです。外すと、元の受入ロットと戻しの受入の両方が「空き」になり、
タンクの残量より多くのロットが引き当てられる状態になります。

なお `input_summary`（使用原酒＝表示用の文）は、明細が動いたら**生きている明細から
組み直します**。手で書いた文が要るときは、直したあとにヘッダの編集で入れ直してください。

**⚠️棚卸について**：シートのB列は「受入」「払出」の2種類だけで、
**3つの台帳のうち原酒だけが棚卸の区分を持っていませんでした**
（資材は「棚卸調整」「欠損」、浄酎は「棚卸調整」「欠減」を持つ）。
受入・払出で代用すると原酒入荷の履歴に棚卸が混ざり、F列の帯の意味も崩れるため、
0019 で専用の区分を足しています。残量ビューは
**棚卸調整を＋（払出先）、欠減を−（受入元）** で数えます。
...
**⚠️G列について**：シートの払出先は自由文で、実データは「黒タンク6」1種類だけ（7件・95.8L）。
タンクマスタに同名の容器は無く（残渣タンクは「残渣保管タンク1〜6」）、
**残渣タンクがどこにも結び付いていなかった**ためタンク別の量が出せなかった。
新システムでは `destination_tank_id` でタンクマスタを参照する。
「黒タンク6」＝残渣保管タンク6（`U-006`）は利用者に確認して紐付けた。
G列の文字は移行データの記録として残すが、**新しい記録では書かない**（タンクIDが唯一の出どころ）。

**⚠️残渣には「出した」記録がありません。** このシートは回収しか記録しておらず、
廃棄・払出の記録がどこにもない。そのためタンク別に出せるのは
**「回収累計」（これまでに入れた量。減らない）**であって、現在量ではない。
画面にもその断り書きを出している。
...
| D | 単位 | 「本」「枚」等 |
| E | 単価(円) | 基準単価 |
| F | ロット数 | 1ロットの入り数（発注の目安）。N列の金額計算に使う。**入荷数を縛るものではない** |
...
**⚠️F列について**：シート上の説明は「この数の倍数でのみ入荷登録できる」でしたが、
**実際の入荷はこの倍数に従っていません**。移行した入荷38件のうち8件が外れており、
正規の発注先である酒井硝子からの分（700mlガラス瓶 36本／ロット15、
ガラス栓 267個／ロット48）も含まれます。正規ルート以外からの仕入れもあります。
セルの値自体も「500（3000）」のような注記付きのものがあり、厳密な発注単位ではありません。
そのため新システムでは**倍数での制限はかけず、発注の目安として表示するだけ**にしています。
