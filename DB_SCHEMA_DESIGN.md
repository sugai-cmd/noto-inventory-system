...
- **資材入荷**：単価・仕入先はマスタから補完する。
  **ロット数の倍数チェックはかけない。** 4-18 F列の説明は「この数の倍数でのみ入荷登録できる」
...
（追記）送料の出し方は 20-3c で変わった。運賃をそのまま送料にするのではなく、
段ボール代と消費税を足して50円単位で繰り上げる。

### 20-3c. 送料 ＝ (運賃 × 荷物 ＋ 段ボール代 × 枚数) × 1.1 の50円繰り上げ

運賃をそのまま送料にしていたので、**段ボール代も消費税も入っていなかった**。
実際の請求に合わせて次の式にした。
...
テープでくっつけて**1つの荷物**として送るので、在庫は2枚減るのに運賃は1回。
24本は12本用を2枚使い、**2つの荷物**として送るので運賃も2回。
1つの数字では表せないので、`carton_rules.material_qty` と `carton_rules.parcels`
に分けて持たせた（既定はどちらも1で、入れるまでは今までどおり動く）。

どちらも `注文本数 ÷ 行の本数` の倍率で伸ばす（24本 ÷ 12本用 = 2）。

**サイズ区分は荷物の外寸（3辺の合計）で決まる。**
`carton_rules` に `length_cm` / `width_cm` / `height_cm` を足した。
くっつけた後の寸法なので、箱単体の性質ではない。だから資材マスタではなく
「商品×本数」の行に持たせている。
外寸が入っていなければ、いままでどおり `carton_size` の手入力を使う。
**どの区分にも収まらないときは手入力に落とさない** — 実際より小さい区分で
安く請求してしまうため、理由を返して金額を出さない。

区分の候補は**運賃表の `DISTINCT carton_size` から作る**（固定の配列にしない）。
運賃表を直せば追従する。

運賃表の金額も段ボールの単価も**税抜**として扱い、合計に一度だけ10%を足す。
税率 `TAX_RATE` は `shippingFeeService` に置き、納品書CSVの消費税もそこから読む
（2か所に書くと改定のときに片方だけ残る）。

`quote()` は金額だけでなく**内訳も返す**。受注画面で根拠を出せないと利用者が検算できない。
...
（追記）その後 `scripts/loaders/cartonRules.js` を作り、`BOX_RULES` の11件を
商品名で名寄せして投入している。4商品ぶんしか無いので、残りは画面から足していく。

### 20-3b. 段ボールを出荷で減らす（`carton_rules.material_id`）

対応表は「商品×本数 → サイズ」だけを持っていて、**どの資材を使うかを持っていなかった**。
そのため段ボール7種（`materials.category = '外箱'`）は資材台帳に1行も記録が無く、
画面の在庫は移行時の初期値のまま止まっていた。
箱詰めのレシピで減るのは化粧箱・桐箱・プラケース（分類は `箱`）という**個装**で、
出荷に使う外装の段ボールはどこでも減っていなかった。

`0023_carton_material.sql` で `carton_rules.material_id` を足し、
「発送済にする」で選んだ段ボールを `material_stock_ledger` に落とす。

- **紐付けは出荷行の `product_ledger_id`。** `ledgerCancelService` は
  この列で資材行を連動取消しており `txn_type` を見ていないので、**取消は無改修で戻る**
- **分類が `外箱` の資材しか受け付けない。** 桐箱は箱詰めで既に減らしているので、
  ここで通すと二重に減る。これが唯一のガード
- 箱数は**推奨を出すだけ**で、実際に使うものは発送画面で選ぶ。
  対応表の行のうち**割り切れる一番大きい本数**を使う（24本なら12本用×2箱）。
  端数の詰め合わせはやらない（組み合わせの選び方に決まりが無いため）
- **複数明細の受注は自動判定の対象外**（対応表が商品×本数の1対1のため）
- 資材不足では止めない（既存の方針。`stockAuditService` が後から拾う）
- 対応表の引き方（`shippingFeeService.findCartonRule`）は**発送画面とゆうパックCSVで共用**する。
  2か所に書くと、画面は「12本用×2箱」と言うのにCSVは「対応表にありません」と出る
...
## 22. 受注の訂正（編集）と、入金予定日の取り違え（22章）

### 22-1. 入金予定日が「当月末日」になっていた原因

実運用で、納品日2026-09-04の受注の入金予定日が **2026-09-30** になっていた。
その得意先の支払いサイトは翌月末日なので、正しくは2026-10-31。

原因は `calcPaymentDueOn` のこの1行だった。

```js
const months = Number.isFinite(Number(termMonths)) ? Number(termMonths) : 0;
```

`Number(null)` は `NaN` ではなく **`0`** になる。つまり得意先マスタの
支払いサイト月数が空欄でも、`Number.isFinite(0)` が真になって
月数0＝**当月**として計算が通っていた。
支払いサイトが未設定なだけなのに、「当月末日」という**それらしい日付が
確定値として保存され**、画面上は正しく見えてしまう。

得意先マスタの登録画面では支払いサイト（月数）の初期値が「（未設定）」なので、
ここを触らずに登録した得意先はすべてこの状態になる。
...
`0`（当月）を明示的に指定した場合はこれまでどおり当月末日になる。
未設定と当月を区別する、というだけの修正である。

発送済にするときの再計算も
`payment_due_on = COALESCE(@paymentDueOn, payment_due_on)` なので、
未設定の得意先では既存の値を消さずにそのまま残す。

### 22-2. 受注の編集

現行シートでは、間違えて入力した行をそのまま書き換えて直していた。
こちらには訂正手段が無く、入金予定日のように**一度保存された誤りを直せなかった**。
受注一覧の各行に「編集」を足した。

| 直せる | 直せない |
|---|---|
| 受注日／納入希望日／納品日／請求日／入金予定日／入金日 | 得意先 |
| 本数／単価／掛率／送料 | 商品 |
| ステータス／販売方法／配送方法／配送先／備考 | 受注番号 |

得意先と商品を変えられなくしているのは、変えると別の受注になってしまうため。
その場合は登録し直す。

気をつけたのは次の3点。

**金額は必ず計算し直す。** 画面から送られてきた売価・合計は使わず、
`単価 × 本数 × 掛率` で組み直す。送料は受注番号に対して1つなので、
`submitOrder` と同じく明細1行目にだけ載せる（2行目以降の行では送料欄を触れなくしている）。
画面側にも同じ式で「直したあとの金額」を出し、確定前に見えるようにした。

**発送済の受注を直したら、商品在庫の出荷履歴も直す。**
本数を12→24に直したのに `product_stock_ledger` が12のままだと、
受注の本数と在庫の減り方が食い違う。同じトランザクションの中で
出荷行の数量・容量・酒税額・日付を合わせる。

**何をどう変えたかを操作ログに残す。**

```
受注 O2609-0001 を訂正した（入金予定日: (空) → 2026-10-31、本数: 12 → 24、
売価: 31680 → 63360、合計: 31680 → 63360）
```

日付欄は**空文字を送ると消える**。間違えて入れた日付を消せないと直しようがないため、
`PATCH /api/orders/:id` の日付項目は `YYYY-MM-DD` と空文字の両方を受け取る。
受注日と本数だけは空にできない（受注として成立しなくなるため）。

画面側は「触った項目だけ」を送る。フォームを開いただけの項目を
送り返さないので、他の人が同時に直した内容を巻き戻してしまうことがない。

### 22-3. テスト

`npm test` は214件（全てpass）。今回の追加は `tests/services/orderEdit.test.js` の13件。

- 支払いサイト未設定の得意先で入金予定日が空になること、設定済みなら従来どおり計算されること
- 発送済にしても未設定なら空のままであること
- 入金予定日を直せること、空文字で消せること、消してから入れ直せること
- 本数を直すと売価と合計が計算し直されること
- 発送済の受注で本数・納品日を直すと出荷履歴も追随すること
- 送料が明細1行目にだけ載ること
- 本数0・受注日空が弾かれること、得意先／商品は送っても無視されること
- 何を直したかが操作ログに残ること

画面はPlaywrightのiPhone 14とPCで通し、編集→保存→一覧の反映まで確認した。

---

## 23. タンクマスタのCSV取り込みと、ファイルのアップロード（23章）

### 23-1. 容器マスタシートの列

現行スプレッドシート「容器マスタ」の見出しをそのまま読み取れるようにした。

| シートの列 | こちらの持ち方 |
|---|---|
| 容器ID | `tanks.code`（必須） |
| 容器名称 | `tanks.name`（必須） |
| 容器種別 | `tanks.container_type` |
| 最大容量(L) | `tanks.max_volume_l` |
| 現在設置場所 | `tanks.location` |
| ステータス | `tanks.status` |
| 検尺定数 | `tanks.gauge_constant` |
| 初期在庫量 | `tanks.initial_volume_l`（**登録時のみ**。23-3参照） |
| 現在液量(L) | **取り込まない**（入出庫の記録から算出する計算列） |
| 理論アルコール度数 | `tanks.current_abv` |
| 備考 | `tanks.note` |

### 23-2. 容器種別が採番の種別名と一致していない

**シートの容器種別と、GASの採番用プレフィックス表のキーは別物だった。**

シートの実データに入っている容器種別は
`蒸留機／ステンレスタンク／木樽／PP／斗瓶／PE／QBテナー` の7種類。
一方、採番に使う `TANK_PREFIXES` のキーは
`ステンレスタンク／木樽／原酒ポリタンク／残渣タンク／一斗瓶／出荷用ポリタンク／QBテナー／蒸留機`。

しかも、容器種別だけでは容器IDのプレフィックスが決まらない。

| 容器種別 | 実際の容器ID |
|---|---|
| PE | `SP-001`（原酒ポリ）と `JP-003`（出荷用ポリ）の**両方** |
| 斗瓶 | `T-004`〜（ステンレスタンクと**同じT**） |
| PP | `U-001`（残渣保管タンク） |

そのため、種別から採番先を機械的に決めることはできない。
取り込みでは容器IDがシートに入っているので採番は不要で、ここは問題にならない。

問題になるのは**手で登録する画面**のほうだった。容器種別が採番表のキーだけを並べた
選択式だったため、取り込んだ `PE` のタンクを編集画面で開くと「（未設定）」になり、
**そのまま保存すると種別が消えていた**。候補は出すが一覧に無い値も入力できる形に変え、
自動採番は採番表にある種別を選んだときだけ動かすようにした。

どちらの呼び方に寄せるかは運用の判断になるので、こちらでは寄せていない。
シートの値をそのまま保持する。

### 23-3. 初期在庫量は登録のときだけ当てる

`初期在庫量` は残量計算の起点なので、既にあるタンクに上書きすると
現在液量が動いてしまう。テンプレートの列定義に `createOnly` を付け、
更新になる行では送らないようにした。画面には
「更新になる行には反映しない列: 初期在庫量」と出す（黙って捨てない）。

### 23-4. 既存行の引き当てをコード優先にした

これまでは名前だけで既存行を引いていた。タンクは容器IDと容器名称の両方がUNIQUEなので、
**容器IDはそのままで名称だけ変えた行**が「新規」と判定され、登録時にID重複で落ちていた。
コード列がある種別では、コード → 名前 の順で引くようにした。

### 23-5. CSVファイルのアップロード

これまでは表計算ソフトで開いてコピーし、画面に貼り付ける必要があった。
ファイルを選ぶ／ドラッグ＆ドロップで読み込めるようにした。

読み込んだ中身は下のCSV欄に入れる。**そのまま取り込まず、目で確かめてから
「確認だけする」→「取り込む」に進む**流れは変えていない。

文字コードの判定は `POST /api/master-import/decode` でサーバー側が行う。
スプレッドシートからのエクスポートはUTF-8だが、**それをExcelで開いて保存し直すと
Shift_JISになる**。取り違えると日本語が全部化けたまま取り込まれてしまうので、
拡張子や利用者の申告ではなく、バイト列だけを見て判定する。

| 判定 | 見分け方 |
|---|---|
| UTF-8 (BOM付き) | 先頭が `EF BB BF` |
| UTF-16LE | 先頭が `FF FE`（Excelの「Unicodeテキスト」保存） |
| UTF-8 | UTF-8として解釈して戻したとき、元のバイト列と一致する |
| Shift_JIS | 上記以外（Shift_JISの日本語はUTF-8として不正なバイト列になる） |

上限は5MB。マスタのCSVが実務上これを超えることはなく、
大きなファイルを丸ごとメモリに載せる必要もないため。

### 23-6. テスト

`npm test` は226件（全てpass）。今回の追加は `tests/services/tankImport.test.js` の12件。
見出しと値は容器マスタシートの実物（`DISTL-01 浄溜機`／`T-001 ステンレスタンク1`／
`U-001 残渣保管タンク1 PP`／`SP-001 原酒ポリ1 PE` など）をそのまま使っている。

- シートの見出しをそのまま取り込め、「現在液量(L)」だけが無視されること
- `PP`／`PE` のような種別を読み替えずにそのまま持つこと
- 容器IDが同じ行は、名称を変えても更新になること
- 更新では初期在庫量を上書きしないこと
- 容器IDと容器名称が必須であること
- UTF-8／BOM付きUTF-8／Shift_JIS のファイルを取り違えずに読めること
- 読み取った中身をそのまま取り込みに渡せること
- 空ファイルがエラーになること、未ログインでは読み込めないこと

画面はPlaywrightのiPhone 14で、Shift_JISのファイル選択 → 確認 → 取り込み →
一覧反映まで通した。
...
GAS版 `BottlingCode.gs` の `getWipLots` / `submitBoxing`（選んだロットを最優先で使い、
足りない分は古いロットから自動で補う）は、20章で `wipLotService` として移植済み。
`/bottling.html` の箱詰めタブに「使う仕掛品ロット」の選択があり、
未指定ならFIFOで自動選択する挙動もGAS版と同じ。

今回足したのは、**その引当の記録が画面から見えていなかった**ぶんの表示と、
液体側（タンク・蒸留・原酒）のロット追跡である。

### 24-2. 液体は混ざるので、割合で追う

タンクの中の液体は物理的に混ざるので、移した後に「この50Lはロット○○」とは言えない。
現行シートの浄酎容器変動履歴でも、容器移動の行に蒸留IDは残らない。

そこで**割合**で持つことにした。

```
T-001 が 100L（D2609-0001が60L / D2609-0002が40L）のとき
  → 50L を B-001 へ移すと
     B-001 に D2609-0001が30L / D2609-0002が20L が入り
     T-001 には D2609-0001が30L / D2609-0002が20L が残る
```

**新しい表は作っていない。** `tank_ledger.distillation_id` は蒸留完了の継足行に
既に入っているので、そこを入口にして台帳を古い順に再生すれば内訳が求まる。

再生の規則は `v_tank_monitor` と同じ（`to_tank_id` で増え、`from_tank_id` で減る）に
合わせてあるので、**内訳の合計は在庫画面の現在液量と必ず一致する**。テストでも突き合わせている。

| 台帳の行 | 扱い |
|---|---|
| 継足（蒸留IDあり） | その蒸留ロットが入口。ロットとして加える |
| 容器移動（from→to） | 移動元の内訳を割合で取り出し、そのまま移動先へ足す |
| 瓶詰（fromのみ） | 割合で取り出す。取り出した内訳を「その瓶詰めの原料」として控える |
| 未納税移出・欠減（fromのみ） | 割合で取り出して捨てる |
| 棚卸調整（toのみ・蒸留IDなし） | 由来が分からないので「棚卸で増えたぶん」として持つ |
| タンクマスタの初期在庫量 | 「移行前からの在庫」として持つ |

### 24-3. `/lots.html`（ロット追跡）

| タブ | 内容 |
|---|---|
| 浄酎タンクの中身 | タンクごとに、どの蒸留ロットが何L・何%入っているか |
| 蒸留ロットの行方 | 蒸留1回＝1ロット。いまどのタンクに何L残り、どれだけ瓶詰めされたか |
| 原酒タンクの中身 | 蒸留の前段。どの銘柄・どの酒蔵の原酒がどのタンクにあるか |
| 仕掛品ロット | 瓶詰め1件＝1ロット。残数・箱詰めの引当先・**元の蒸留ロット** |

仕掛品ロットのタブでは、瓶詰めロットから**元の蒸留ロットまで遡れる**。
`L2609-0001（100本）← T-001 ← D2609-0001が18L・D2609-0002が12L` のように出る。
1回の瓶詰めが複数の蒸留ロットにまたがるときは、本数もその割合で按分して見せる。

### 24-4. 選ぶときにも中身が見えるようにした

一覧を見に行かないと分からないのでは、瓶詰めや容器移動のときに使えない。
`/bottling.html` の払出元タンクと、`/tanks.html` の移動元・移動先タンクを選ぶと、
その場に中身の内訳を出すようにした。

```
現在液量 50.0L / 最大 245.0L・度数 -%
中身: D2609-0001 30.0L（60.0%） ／ D2609-0002 20.0L（40.0%）
```

内訳の取得は `Promise.allSettled` にしてある。ロット追跡が取れなくても
瓶詰め・容器移動そのものは登録できるようにするため。

### 24-5. 原酒側は台帳が別

原酒は蒸留の前段なので `raw_sake_ledger`（受入／払出）で別に持っている。
こちらは蒸留ロットではなく**銘柄（＋酒蔵）**が単位になる。
銘柄マスタに無い受入は、原酒スペックか受入元の自由記述をそのまま見出しにする
（勝手に銘柄マスタへ足すと、表記ゆれの分だけマスタが増えるため）。

### 24-6. テスト

`npm test` は236件（全てpass）。今回の追加は `tests/services/lotTrace.test.js` の10件。
2ロットを同じタンクへ継足 → 容器移動 → 瓶詰め → 箱詰め、という実際の流れで検証している。

- 継足した蒸留ロットがタンクの中身として出ること
- 容器移動で中身の割合がそのまま移ること（60:40が移動先でも60:40になる）
- **内訳の合計がタンクモニターの現在液量と一致すること**（全タンクで突き合わせ）
- 蒸留ロットの行方が、残っているタンクごとに出ること
- 瓶詰めが使った蒸留ロットが分かること（30Lが18L:12Lに按分される）
- 蒸留ロット側からも瓶詰め済みのぶんが見えること（100本の18/30＝60本相当）
- 箱詰めの引当先と残数が出ること
- 原酒タンクの中身が銘柄と酒蔵で出ること

画面はPlaywrightのPCとiPhone 14で4タブすべてと、瓶詰め・タンク操作の
中身表示まで通した。

---

## 25. 過去データの移行 — 実データを通して分かったこと（25章）

移行スクリプトと16本のローダーは8章で用意していたが、**実データを一度も通していなかった**。
現行スプレッドシート（21シート・約1,500行）を実際に流したところ、
このまま本番投入すると数字が壊れる箇所が見つかった。ここに残す。

手順書は `docs/MIGRATION.md`。

### 25-1. 数字が壊れる3つの原因

**(1) 浄酎容器変動履歴の数量が負のまま入る**

シートは向きを**符号**で表す。瓶詰め・容器移動の行は `-31.95` のように負。

```
| 2026/05/21 | ステンレスタンク3 | 瓶詰   |    | 直接充填         | -31.95 |
| 2026/06/12 | ステンレスタンク1 | 容器移動 |    | 一斗瓶2          | -19.9  |
| 2026/06/16 | 蒸留機           | 継足   |    | ステンレスタンク1 |  15.0  |
```

こちらは**受入元／払出先の列**で向きを表し、`v_tank_monitor` は
`from_tank_id = t.id → -quantity_l` で引く。負の値が入ると `-(-31.95)` となり、
**瓶詰めするとタンクが増える**。瓶詰め・容器移動の全行が逆向きになっていた。

→ `parseAbsNumber` で大きさだけを取る。向きは列が持っているので符号は冗長。

**(2) 桁区切りのカンマが数値にならない**

シートの数量・金額は `1,000` `31,680`。ローダーは素の `Number()` を使っており、
`Number('1,000')` は **NaN**。NOT NULL列なら行が落ち、そうでなければ黙ってNULLになる。
度数の `36.55%` も同様。

→ `scripts/lib/parseNumber.js` に読み取りを集約。カンマ・全角・`%`・通貨記号・
末尾の `L` を落とし、空欄と `-` はnull、読めない値は例外にする（黙って0にしない）。

**(3) 移行スクリプトがシートのエクスポートを受け取れない**

マスタ画面のCSV取り込み（23章）でシート側の列名・並び順・タブ区切り・Shift_JIS に
対応したが、**その改修は移行スクリプト側に入っていなかった**。

→ `scripts/lib/csvReader.js` を `masterImportService.decodeUpload` /
`detectDelimiter` 経由にし、`scripts/lib/columnAliases.js` で見出しの別名を吸収する。
別名の表はマスタ画面の `TEMPLATES` から作るので、画面と移行で条件が食い違わない。
ローダーは `row['資材名']` のまま書けばよく、シートが「資材名称」でも読める（Proxyで解決）。

### 25-2. 同じ伝票番号が2つの意味で使われていた

| 組 | 完全一致した件数 |
|---|---|
| 受注番号（D）↔ 蒸留ID（D） | **37件**（蒸留ID43件のうち） |
| 原酒受払ID（M）↔ 資材履歴ID（M） | **73件** |

参照は別々の表で引くので取り込みは通るが、人が見て区別できない。
移行時に新しい採番規則へ振り直す。

| 過去 | 移行後 |
|---|---|
| 受注番号 `D2605-0001` | `O2605-0001` |
| 原酒受払ID `M2603-0001` | `R2603-0001` |
| 蒸留ID `D2606-0001` / 資材履歴ID `M2506-0001` | そのまま（記号が正しい） |

元の番号は `orders.legacy_order_no` / `raw_sake_ledger.legacy_lot_code` に残す。
名寄せの表は**元の番号で持つ**（他シートが元の番号で参照しているため）。

副産物として採番の連続性も直る。振り直さないと2608の新規受注が `O2608-0001` から
始まり、過去の `D2608-0006` と番号が並んで見える。振り直せば `O2608-0007` から続く。

全シートの自由記述欄を走査したが、**文章の中に伝票番号が書かれている行は0件**だった。

### 25-3. スキーマ側の不足（`0013_migration_readiness.sql`）

| 直したこと | 理由 |
|---|---|
| `product_stock_ledger.txn_type` に `未納税移出` を追加 | 実データに3件。瓶詰めロットからの払い出しなので**仕掛品**を減らす（受入元が瓶詰めの商品履歴ID、保管場所が熟成室であることから判断） |
| `consignment_reports` に `report_no`、`order_id` を任意に | シートの「受注番号」列は `C2606-0001` という**この報告自身の番号**で、受注リストの番号ではなかった（18件すべてC始まり） |
| `sample_shipments.product_id` を任意に | シート名のとおり「販促資料」だけを送った行が2件ある。商品が無いという理由で送付の記録を落とさない |
| `customers` に連絡先11列 | 顧客リストシート（90行）のメール・電話・郵便番号を取り込むため |
| `orders` / `raw_sake_ledger` に旧番号の列 | 25-2 |

**顧客リストの掛け率は取り込まない。** `60` `80` の％表記で、
得意先マスタの `0.6` `0.8` とは別物。掛け率と請求・入金の期日は得意先マスタを正とする。

### 25-4. 実データの傷を、行を落とさずに扱う

| 実データの状態 | 扱い |
|---|---|
| 資材マスタのロット数が `500（3000）` | 先頭の数値を取り、元の書き方を備考へ残す。この1セルで行が落ちると、その資材を使うレシピ2行も芋づるで落ちていた |
| 同じ受注番号に別の得意先（6件） | 明細2行目として残す。番号の付け間違いなので、移行後に受注一覧の編集で直す |
| 同じ商品履歴ID・資材履歴ID（14件） | `M2607-0013-2` のように枝番を付けて**行は残す**。落とすと在庫の動きが1件消える |
| 蒸留記録に無い蒸留ID（3件） | 容器の動きは残し、蒸留への紐付けは空にする |
| 廃番の商品・資材を指すレシピ行 | `aliases.json` の `__ignore__` で「マスタに無いのが正しい」と宣言し、静かに飛ばす。不一致として数えると本当の不一致が埋もれる |
| 蒸留日・残渣回収日に時刻が無い | `00:00` で埋める（NOT NULL列。移行分は完了済みなのでアラートに影響しない） |
| 日付欄の `-` `未定` | 空欄として扱う。日付が無いだけで行を落とさない |
| 対象月が `26年7月` | `2026-07` として読む |

### 25-5. シートのモニターは台帳と一致しない（重要）

答え合わせのつもりで商品在庫モニター・資材在庫モニターと突き合わせたところ、
**大きく食い違った**（例: 300mlガラス瓶 シート80 / 台帳から計算1680）。

原因は計算違いではない。GAS版の `updateStockMonitor_` は

```js
sheet.getRange(i + 1, targetCol + 1).setValue(current + delta);
```

と、操作のたびにモニターのセルへ直接足し引きしている。
**台帳から計算し直しているのではなく、別建てで持っている数字**なので、
記録漏れや手修正のぶんだけ台帳とずれていく。実際、シートの仕掛品には `-22` という
マイナスの値も残っていた。

初期在庫の二重計上を疑って検算したが、そちらは問題なかった
（300mlガラス瓶は初期在庫0、台帳の入荷2640・消費960で1680）。

**新システムは台帳から計算する。** したがって移行後の在庫はシートのモニターと一致しない。
これは不具合ではなく設計の違いなので、`scripts/verify-migration.js` で差を並べて出し、
**棚卸で実際に数えた数に合わせる**運用にする。棚卸の調整は台帳に記録として残るので、
以降は計算値と実物が一致する。

残量がマイナスになっているタンクも同じ理由で出る（払出が入庫を上回っている）。
verify-migration.js が一覧で出すので、棚卸の対象にする。

### 25-6. 蒸留の「使用原酒明細」

蒸留記録シートのこの列は、人が読む文章ではなく **`DTL-0001 DTL-0002` という明細IDの羅列**。
明細記録を経て原酒のスペックまで辿るための紐付けで、実データで検証したところ
**79件すべて**が `使用原酒明細 → 蒸留明細記録 → 原酒受払記録 → 原酒スペック` まで解決し、
参照切れ・蒸留IDの食い違い・孤立した明細はいずれも0件だった。

DBはこの関係をシートより素直に持てる（`distillation_details.distillation_id` が
正式な外部キーなので、明細が何件ぶら下がっても表せる）。必要なのは次の2点だけだった。

- 明細IDを `detail_code` へ取り込む（先日この列を足したのにローダーが読んでいなかった）
- 一覧の「使用原酒」欄は明細から組み直す（タンク名＋投入量L）。
  そのまま入れると `DTL-0001 DTL-0002` と表示されて読めない

### 25-7. 予行演習の結果

こちらで実データのCSVを作り、`--dry-run` を繰り返して上記を全部直したうえで通した。

```
得意先マスタ 156 / 顧客リスト 90 / 商品マスタ 21 / 資材マスタ 33 / タンクマスタ 68
製品レシピ 41 / 酒蔵 3 / 原酒 6 / 段ボール対応表 11
受注リスト 117 / 蒸留記録 43 / 原料受払 158 / 蒸留明細 79 / 残渣回収 7
商品在庫変動履歴 206 / 資材在庫変動履歴 206 / 浄酎容器変動履歴 93
委託販売実績報告 18 / サンプル送付 49 / 売上目標 2

PRAGMA foreign_key_check: 0件
```

移行後のロット追跡も動く。原酒タンクに「BYR6浄酎用池月（鳥屋酒造）」「西之門　雲山」が出て、
浄酎タンクの中身が蒸留ロット別に分かれる（継足の行が蒸留IDを持っているため）。

### 25-8. テスト

`npm test` は253件（全てpass）。`tests/services/migrationLoaders.test.js` の16件を追加。

- カンマ・全角・`%`・空欄・`-` の読み取り、読めない値で例外になること
- 符号付きの数量から大きさだけを取ること
- `500（3000）` を救い、元の書き方を残すこと
- 伝票番号の振り直しと、形が違う番号を作り変えないこと
- 重複した履歴IDに枝番が付き、系統ごとに別勘定になること
- シート側の列名・全角半角の違いで読めること
- `26年7月` の対象月、日付欄の `-`、時刻の無い日付
- Shift_JIS とタブ区切りのCSVが読めること
- **通しの確認**: `-19.9` の容器移動1行を実際に投入し、移動元が19.9L減って
  移動先が19.9L増えること（直す前は逆になっていた）
...
カナカンのように支店を持つ会社が、得意先マスタでは**バラバラの行として並んでいる**。

| 顧客ID | 得意先名 | 掛率 | 支払いサイト | 受注件数 |
|---|---|---|---|---|
| C0008 | カナカン酒類石川 | 0.7 | 翌月末日 | 14件 |
| C0009 | カナカン酒類七尾 | 0.7 | 翌月末日 | 8件 |
| C0010 | カナカン酒類高岡 | 0.7 | 翌々月末日 | 1件 |
| C0101 | カナカン酒類富山 | 0.7 | **（空欄）** | 0件 |
| C0105 | カナカン酒類福井 | 0.7 | **（空欄）** | 1件 |
| C0151 | カナカン業本七尾 | 0.7 | 翌月末日 | 0件 |

一方、得意先の連絡先の表には「カナカン」が1行あり、住所は金沢市彦三町の本社、
請求書送付先は「業務課」、入金期日は「翌々月31日」。明らかに**本店**の情報だった。

実務は **請求・与信・担当は本店単位、受注・納品は支店単位**で回っている。
この関係がデータベースに無いため、次のことが起きていた。

- **カナカン富山・福井は支払いサイトが空欄** → 受注を入れても入金予定日が出ない
  （22章で直した「入金予定日が当月末日になる」のと同じ場所）
- 本店の請求先・担当者を置く場所が無い（移行では「カナカン」の行を捨てていた）
- 「カナカン全体でいくら売れたか」が出せない
...
**取引先グループの表を別に作らなかった理由**

- **本店にも直接受注が来る。** 本店を「グループ」という別概念にすると本店宛の受注を
  入れられない。本店も得意先の1つである方が実態に合う
- グループ表にすると「グループに属さない普通の得意先」の分岐が
  受注・請求・集計の全部に入る。150件のうち親子があるのは数社なので割に合わない
- 3段（本社→支社→営業所）になっても同じ仕組みで伸びる
- 既存の受注・請求・CSV出力は `customers` を見ているので、そのまま動く

### 26-3. 継承するのは請求まわりだけ

`src/services/customerService.js` の `resolveBilling()` に集約した。
**支店の欄が空のときだけ**上を見る。支店に値が入っていれば支店が勝つ。
1段だけでなく**根まで辿る**ので、3段以上でも効く。

| 継承する | 継承しない |
|---|---|
| 支払いサイト（月数・日） | 住所・郵便番号・電話 |
| 請求書送付期日 | 得意先名・顧客ID |
| 請求書送付先メール・担当者氏名 | 業態・区分 |
| 掛率 | 配送先 |

住所や電話は支店ごとに違うので継承しない。掛率は本店との取り決めなので継承する。

呼び出し元は `orderService`（受注登録の初期値・発送済の再計算・掛率の既定値）と
`quotationService`。`calcPaymentDueOn` 自体は変えず、**渡す値を解決してから呼ぶ**だけにした。

戻り値には `inheritedFrom`（項目名 → 引き継ぎ元の得意先名）が付く。
画面で「なぜ空欄なのに入金予定日が出るのか」を説明するために使う。

### 26-4. 掛率を「未設定」にできるようにした（0014）

継承を実装しようとして分かったこと。`customers.markup_rate` は
**`NOT NULL DEFAULT 1`** だったため、「まだ決めていない」と「1.0（上代どおり）」を
区別できなかった。掛率1は消費者向けで実在する値なので、1を未設定とみなすこともできない。

これだと**支店を新しく登録したときに掛率が黙って1.0（上代どおり）になる**。
卸相手なら大きな値付けの誤りで、22章で直した「支払いサイトが空でも当月末日になる」のと
同じ形の問題だった。テーブルを作り直して空を空のまま持てるようにした。

使う側は既に `markup_rate ?? 1` で受けているので、挙動は変わらない。
`customerModel.create` が入れていた既定値の `?? 1` も外した。

### 26-5. 画面

**マスタ登録** — 得意先タブに「本店（親の得意先）」欄。自分自身と、
本店と支店が輪になる指定（`assertValidParent`）を弾く。
編集中は自分自身を選択肢から外す。引き継いでいる値をその場に出す。
...
**得意先検索** — 検索の対象に本店名を含めた。「カナカン」と打つと6支店が全部出る。
どれがどの会社の支店か分かるよう、結果に `本店: カナカン` を併記する。
支払いサイトが空欄の支店には `（本店から）` と出す。

**一覧** — 「本店」の列を足した。

**注意**: `renderForm()` は `loadList()` より先に走るので、
本店の選択肢は一覧の読み込み後に `refreshParentOptions()` で入れ直している。

### 26-6. CSVの取り込み

得意先マスタのテンプレートに **「本店」列**（別名: 親得意先／本社／本店名）を足した。
シートには無い列なので任意。名前で親を引く。

**取り込みは2周する。** 本店の行がCSVの後ろにあることがあるので、
全行を入れ終わってから親を紐付ける（`linkCustomerParents` / `linkParents`）。

`customerModel.update` の `parent_id` は `COALESCE` にしていない（本店を外す操作があるため）。
ただし CSV取り込みのように項目を送らない更新で消えないよう、
**渡されたときだけ書き換える**（`parentIdProvided`）。

### 26-7. どの会社が親子かは機械では決められない

シートに親子関係が書かれていないので、移行スクリプトでは設定しない。
移行後に画面から本店の行を作り、各支店で選んでもらう運用にした（`docs/MIGRATION.md`）。

### 26-8. この変更で起きないこと

- **過去の受注は変わらない。** 単価・掛率・入金予定日は登録時にスナップショットで
  保存しているので、あとから本店を設定しても過去の金額は動かない
- 本店を設定しない得意先は今まで通り（`parent_id = NULL`）

### 26-9. テスト

`npm test` は265件（全てpass）。`tests/services/customerParent.test.js` の12件を追加。

- 支払いサイトが空欄の支店が本店の値を引き継ぐこと、支店に値があれば支店が勝つこと
- 住所は継承しないこと、本店が無い得意先はこれまで通りであること
- **空欄だった支店の受注で入金予定日が出ること**（`2026-11-30`。これが直したかったこと）
- 発送済にしたときの再計算も本店から出ること
- 本店の名前で支店を検索でき、結果に本店名が付くこと
- 自分自身を本店にできないこと、輪になる指定を弾くこと
- 本店を外せること、**本店を送らない更新で本店が消えないこと**
- 3段（本社→支社→営業所）で根まで辿ること
...
### 27-7. 明細を差し替えたとき、一覧と明細で表示が食い違っていた

利用者の報告:「D2609-0006の蒸留記録を修正したんですが、**修正内訳には反映されている
んですけど、受払一覧には反映されてない**ので両方とも同じ表示になるように」。

原因は、`updateDistillationDetail` / `cancelDistillationDetailItem` が
**元の払出行をそのまま残す**こと。打ち消しは「戻しの受入」を別に足す作りなので、
台帳の行自体は `is_cancelled = 0` のまま残る（消すと残量が二重に戻る）。
そのため投入明細が「取消済み → DTL-99」と出している行を、受払一覧は「有効」と
言い続けていた。

調べたところ、関係する直し漏れが3つあった:

| | 症状 | 直し方 |
|---|---|---|
| A | 受払一覧で、差し替えられた払出が「有効」のまま | 一覧に `detail_cancelled` / `detail_code` / `detail_note` を足し、明細の状態で出し分ける |
| B | 蒸留記録一覧の「使用原酒」が古いまま | `rebuildDistillationInputs()` で、生きている明細から `input_summary` と `total_input_l` を組み直す |
| C | 差し替えで作った払出に引き当てが付かない | `updateDistillationDetail` にも `allocateForPayout` を足す（開始・明細追加では呼んでいたのに、ここだけ抜けていた） |

**データの持ち方は変えていない**（利用者の判断）。稼働中のシステムで、残量の出し方に
手を入れると影響が大きいため。表示を揃えるだけなら過去の記録もその場で揃う。

一覧の言葉は投入明細の画面に合わせた:

- 明細が取消済みの払出 … **「取消済み」**（行は muted、明細の備考「修正のため差し替え →
  DTL-99」までそのまま出す）
- 打ち消しの戻し受入 … **「戻し」**。受入に `distillation_id` が付くのはこれだけなので、
  原酒入荷の受入と1つの条件で見分けられる
- 差し替えられた行に「未引当」は出さない（直しようがないため）

**引き当ては外さない。** 差し替えられた払出は残量を引いたままで、戻しの受入が
足し返している。引当を外すと、元の受入ロットと戻しの受入の両方が「空き」になり、
タンクの残量より多くのロットが引き当てられる状態になる。

### 27-8. 既に直してあった記録の組み直し（0022）

27-7 の B（使用原酒が古いまま）をサービス側で直したが、`rebuildDistillationInputs()`
が走るのは**これから明細を動かしたときだけ**。利用者が既に直していた D2609-0006 は
二度と組み直される機会がなく、一覧は「原酒ポリ1 20L / 原酒ポリ3 10L」のままだった。

0022 で、**取消済みの明細を持つ蒸留だけ**を組み直す。

- 直していない記録まで書き換えると、移行時にシートから取り込んだ文
  （「原酒ポリ5 10.0L / 原酒ポリ6 20.0L」）まで作り直すことになる
- **生きている明細が1件も無い蒸留は対象外**。組み直すと空文字になってしまう
- 数量の書き方はサービス側（JavaScript）と揃える。SQLite の REAL をそのまま連結すると
  `20.0` が「20.0L」になり、これから登録されるぶんの「20L」と食い違う。
  `RTRIM(RTRIM(printf('%.6f', x), '0'), '.')` で `String(20)` と同じ「20」になる

リハーサルDBで確認: 取消済みの明細を持つ蒸留は0件なので**43件すべて変化なし**。
利用者と同じ形（明細を2件とも差し替えてから使用原酒を古い文に戻した記録）を作って
流すと、その1件だけが「原酒ポリ10 20L / 原酒ポリ11 10L」に組み直された。

残った課題（今回は見送り）: 戻しの受入が**新しいロット番号として増える**ため、
差し替えのたびに元のロットの同一性が失われる。記録の付け方そのものを変える案
（戻しを作らず元の払出を取消にする）は、過去分の移行が要るため別PRとする。

---

## 28. 一覧の件数選択・ページ送り・絞り込み（PR C-1）

### 28-1. なぜ必要か

一覧が長くなってきたので件数選択・ページ送り・絞り込みを入れる。調べると、
**使い勝手の前に「いま見えていない行がある」画面が2つあった。**

| 画面 | 実データ | 何が起きていたか |
|---|---|---|
| 資材の入出庫履歴 | **205件** | 既定200で切れ、**5件が画面に出ていなかった** |
| 操作ログ | 増え続ける | 画面が `?limit=100` を決め打ち。**古いログは一切見られない** |

資材のほうは、サービスの既定値・APIの既定値・画面が `limit` を送っていないことの
3つが重なっていた。**瓶詰めタブ（`ledgerCancelService`）で「直近30件しか見えない」と
分かったのとまったく同じ形**で、あのとき直したのがその画面だけだった。

どちらも**一覧の取得に試験が1つも無かった**。取消・編集（PATCH/POST）の試験はあったが、
GETの形を誰も見ていなかったので、200件の壁に当たっても気づけなかった。

### 28-2. 手本をそのまま写した

- サービス: `ledgerCancelService.listLedgerRecords`
  （limit / offset / sort の**許可リスト** / order / 絞り込み / `{rows, total}`）
- 画面: `public/bottling.html` の `historyView`

**画面から来た文字列をORDER BYに入れない。** `SORTABLE` に載っている名前だけを通し、
それ以外は既定に落とす。実在する列（`l.id` や `material_id`）でも、許可リストに
無ければ通さない。

### 28-3. 共通にしたところ／していないところ

`public/assets/js/app.js` に置いたのは**ページ送りと並べ替えだけ**
（`createListView` / `listQuery` / `renderSortMarks` / `renderPager` /
`wirePager` / `wireSortHeaders`）。

**絞り込みは各画面に残した。** 一覧ごとに項目が違うので、1つの関数へ押し込むと
どの画面が何を送るのか読めなくなる。

### 28-4. 決めたこと

- **絞り込みを変えたら1ページ目へ戻す。** 3ページ目で絞り込むと、該当0件なのに
  「次へ」だけ押せる状態になる
- **取消・編集のあとは、開いていたページに留まる。** `loadLedger()` は offset を
  触らない（10件ずつの3ページ目で取り消しても、1ページ目に飛ばない）
- 操作ログの「操作の種類」の選択肢は、**実データにある `action` から作る**
  （`GET /api/auth/operation-logs/options`）。固定リストにすると、新しい操作を
  足したときに絞り込めない行が生まれる
- 並べ替えは `id` を第2キーにする。同じ値の行が、実行のたびに入れ替わらないように

### 28-5. 返り値の形が変わった

`materialService.listLedger` と `operationLogService.list` は、行の配列ではなく
`{rows, total}` を返すようになった。`total` は**同じ絞り込みでの全件数**
（ページ送りの「N件中 a〜b件」に使う）。

呼び手は routes 2箇所と、`tests/services/security.test.js` の操作ログの試験だけだった。

### 28-6. テスト

`npm test` は621件（全てpass）。`tests/services/listPaging.test.js` の14件を追加。

- **205件すべて見られること**（`limit=50&offset=200` で201〜205件目、`total`=205）
- ページを送っても**重複も抜けもない**こと
- 絞り込むと `total` もその条件の件数になること（全件数のままにしない）
- **許可していない並べ替えを送っても壊れず既定順**で返り、表が消えていないこと
  （`'quantity; DROP TABLE materials'` など5通り）
- `order` に変な値、`offset` に負の数を送っても落ちないこと
- 取り消したあと、同じ条件で読み直すと有効な件数が1件減ること
- **操作ログが100件を超えて遡れること**
- 選択肢が実データの `action` と一致し、**まだ使われていない操作は出ない**こと

7か所（許可リスト×2・offset×3・totalの絞り込み・選択肢の作り方）について、
外すと対応する試験が落ちることを確認した。

ブラウザ: 資材の3ページ目に5件出る／並べ替えで1ページ目に戻る／絞り込みを重ねると
件数が 205→38→30 と減る／10件ずつの3ページ目で取り消しても**そのページに留まる**／
操作ログが150件中101〜150件まで遡れる。JSエラー無し。

### 28-7. まだやっていない（C-2 / C-3）

同じ形を広げる先: 受注一覧117件・原料受払記録158件・修正履歴（C-2）、
得意先156件の検索窓・タンク台帳93件・サンプル出荷49件・蒸留記録43件（C-3）。

## 28-8. 受注・原料受払・修正履歴にも広げた（PR C-2）

28-1〜28-7 で作った形を、次の3つに写した。**新しい設計はしていない。**

| 一覧 | 実データ | 以前の状態 |
|---|---|---|
| 受注一覧 | 117件 | **offset も総件数も無く、ORDER BY も固定** |
| 原料受払記録 | 158件 | #45 で作ったばかり。区分・タンク・取消済みでは絞れた |
| 修正履歴 | — | `limit = 200`、絞り込みは対象コードのみ |

どれも既定200にはまだ当たっていないが、増え続けるので先に揃えた。
**3つとも一覧の取得に試験が無かった**（GETの形を誰も見ていなかった）。

### 決めたこと

**受注は日付が3種類ある**（受注日・納品日・入金予定日）。`from`/`to` を3組に増やすと
画面もAPIも読みにくくなるので、**どの日付で絞るかを `dateField` で選ぶ**形にした。
列名は `DATE_FIELDS` の許可リスト経由で、知らない名前が来たら受注日に落ちる。

**CSV出力も同じ絞り込みが効くようにした。** 画面のボタンが「上の絞り込み条件で出力」と
言っている以上、一覧で絞れてCSVで絞れない項目があると嘘になる。
`csvExportService.fetchOrders` に `customerId` / `productId` / `dateField` を足し、
`routes/exports.js` の `parseFilter` でも通す。
**受注一覧に絞り込みを足すときは、こちらも一緒に足すこと。**

**修正履歴は寄せ集め（UNION）なので、並べ替えられる列が限られる。**
数量や金額は枝ごとに意味が違うため、そもそも列になっていない。
また**蒸留明細の枝だけ `occurred_at` が NULL**（取消の日時を残さず、理由を備考に置く作り）。

- 日付で絞るとその枝は落ちる。**黙って消さず、画面に断りを出す**
- 並べ替えでは昇順・降順のどちらでも**最後に置く**（`NULLS LAST`）。迷子にしない

### 返り値の形が変わった

`orderModel.list` / `rawSakeLedgerService.listLedger` /
`correctionHistoryService.list` が `{rows, total}` を返すようになった。
呼び手は routes 3箇所と試験6箇所だった。

### テスト

`npm test` は632件（全てpass）。`tests/services/listPaging.test.js` に11件を追加
（合計25件）。**11か所について、外すと対応する試験が落ちることを確認**
（許可リスト×4・offset×4・totalの絞り込み×2・`NULLS LAST`）。

空振りした書き方を2つ直している:

- 修正履歴の `total` を絞り込み前にしても落ちなかった。取消行を全部同じ日付で
  作っていたため、絞っても件数が変わらなかった → **月を2つに分けた**
- CSV出力の絞り込みを外しても落ちなかった。種の「発送済」が全部同じ得意先に
  偏っていた → **両方の得意先にまたがる「未着手」で確かめる**形にした

ブラウザ（リハーサルDBの実データ）:
受注117件→商品で70件→納品日で53件（受注日だと48件で、日付の切替が効いている）→
得意先「カナカン酒類石川」で14件。原料受払158件を100件ずつ2ページ、7月以降で62件。
修正履歴の断り書きも出る。JSエラー無し。

## 28-9. 版が食い違ったときに、原因が分かるようにした

利用者の環境で `Cannot read properties of undefined (reading 'length')` が
**4画面に同時に出た**（資材・受注・蒸留・在庫監査）。

原因は **`git pull` したあとサーバーを再起動していなかった**こと。
`public/` は `express.static` がディスクから毎回読むので pull した瞬間に新しくなるが、
`src/` は**起動時に読まれたものが動き続ける**。古いサーバーは行の配列を返すため、
新しい画面が `{rows, total}` に分解すると `rows` が undefined になり、
`rows.length` で落ちていた。

**逆向き（古い画面＋新しいサーバー）では例外にならない**（`{rows,total}.length` は
undefined になるだけで、一覧が空に見える）。この非対称が切り分けの手がかりになる。

存在しないAPIには `src/app.js` が同じ趣旨の案内を404で返していたが、
**形が変わっただけでは404にならず素通りしていた**のがこの穴。

`public/assets/js/app.js` の `asListResult(res, endpoint)` で塞いだ。
**一覧APIを `{rows, total}` に変えるときは、画面側でこれを通すこと。**

### 止めずに、出したうえで知らせる

最初は「原因の分かるメッセージで**止める**」形にしたが、それでは**一覧が1行も
出ないまま**で、利用者の仕事が止まる（実際にそう報告された）。

行そのものは返ってきているのだから、**更新前と同じ見え方で出す**。
効かないのはページ送り・総件数・新しく足した絞り込みだけなので、
そこは黄色い帯（`.msg.warn`）で知らせる。赤いエラーにはしない。

- 総件数はいま届いている行数で代用する（`{ rows, total: rows.length, stale: true }`）
- 帯は**1回だけ**出す。画面を触るたびに出し直しても仕方がない

試験（`tests/services/listPaging.test.js`）:

- 5つの一覧APIが**配列ではなく `{rows, total}`** で返ること
- `asListResult` が古い形でも**行をそのまま出し**、`warn` の帯で
  **どの一覧か＋再起動**を知らせ、2回目は出し直さないこと

試験は `public/assets/js/app.js` を読み込んで評価する（書き写すと本体とずれる）。
`showMessage` は同じファイル内の関数宣言なので**引数では差し替えられない**
（宣言が引数を上書きする）。`document` を偽物にして、素の `showMessage` を
そのまま走らせている。
