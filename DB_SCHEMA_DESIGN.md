...
古い形（配列）が返ってきたら、どの一覧かと「再起動してください」を書いた
メッセージで止める。**一覧APIを `{rows, total}` に変えるときは、
画面側でこれを通すこと。**

試験（`tests/services/listPaging.test.js`）:

- 5つの一覧APIが**配列ではなく `{rows, total}`** で返ること
- `asListResult` が古い形を、**どの一覧か＋再起動**が書かれたメッセージで断ること
  （`public/assets/js/app.js` を読み込んで評価する。書き写すと本体とずれる）
