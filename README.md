...
| `/orders.html` | 受注登録・一覧（絞り込み・並べ替え・ページ送り）・編集（訂正）・発送済処理・CSV出力 |
| `/bottling.html` | 瓶詰め・箱詰め |
| `/distillation.html` | 原酒入荷・蒸留開始・完了報告・原料受払記録の一覧／編集／取り消し・原酒ロットと蒸留の引き当て |
...
| `/stocktaking.html` | 棚卸（商品・仕掛品／資材／タンク（浄酎）／タンク（原酒）） |
| `/shipments.html` | 返品・サンプル送付・委託販売実績報告 |
| `/materials.html` | 資材入荷・資材在庫・入出庫履歴（取り消し・編集・絞り込み・ページ送りつき） |
| `/audit.html` | 在庫監査レポート・修正履歴（絞り込み・ページ送りつき） |
| `/sales-targets.html` | 売上目標と進捗 |
| `/quotations.html` | 見積管理（売価・利益・確度をかけた見込み） |
| `/masters.html` | マスタ登録・編集（得意先・商品／製品レシピ・資材・タンク・酒蔵・原酒）・CSV一括登録（ファイル選択／ドラッグ＆ドロップ対応） |
| `/shipping.html` | 送料設定（都道府県の地帯・料金表・段ボール対応表）。対応表の「段ボール資材」を設定すると、発送時に在庫から引く箱が決まる |
| `/settings.html` | 2要素認証・パスワード変更・利用者管理・操作ログ（絞り込み・ページ送りつき） |
...
| 資材履歴ID | `M2608-0001` |
| サンプルID | `S2608-0001` |
| 原酒受払ID | `R2608-0001`（払出・棚卸）／`R2608-1001`（1回目の移入）／`R2608-2001`（2回目の移入） |
